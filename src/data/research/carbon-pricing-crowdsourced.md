---
title: Fifty-Five Crowd-Sourced Designs Assessing Carbon Pricing Support
description: A large-scale "many-designs" project testing 55 independently developed behavioral interventions on real-world support for carbon pricing.
tags:
  - Behavioral Economics
  - Climate Policy
  - Large-Scale Collaboration
status: Conditionally Accepted
authors: Holzknecht, Schwaiger, Blanco, Huber, Kirchler & 55 co-authors (incl. Raphael Gottweis)
updated: "2026-06-23"
link: https://manydesignscarbon.online
order: 3
---

Conditionally accepted at *Nature Human Behaviour*. A carbon price is an effective, cost-efficient way to cut emissions, but low public acceptance keeps holding back real-world adoption. This pre-registered "many-designs" project brought together international research teams — my team among them — who each independently designed a behavioral intervention meant to increase support for carbon pricing. All 55 interventions were then tested simultaneously with almost 20,000 U.S. residents.

The interventions moved support only slightly (small but statistically significant effects), with fairly low heterogeneity across the very different designs teams came up with. One of the more interesting findings: researchers were substantially overconfident about how well their own — and their peers' — interventions would work, a useful data point on how the field calibrates its expectations.
